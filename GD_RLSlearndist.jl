...
@variables (dθ(t))[1:k,1] (θ(t))[1:k,1] (Ψ(t))[1:k,1] (P(t))[1:k,1] (tmp(t))[1:k,1]
@parameters γ0=8
γ = 8*ones(k)
#γ = [5 5 1 1 1 5]
...
@named UpdateLaw = ODESystem([
        [D(θ[i]) ~ γ[i] * P[i] * Ψ[i] * (V-Vh) for i=1:k]...,
        #[D(θ[i]) ~ IfElse.ifelse(θ[i] <= 0, max(dθ[i],0), dθ[i]) for i=1:k]...,
        [D(Ψ[i])  ~ -γ[i]*Ψ[i] + ϕ[i] for i=1:k]... 
],t)

@named dP = ODESystem([
    [D(P[i]) ~ (α*P[i] - α*P[i]*Ψ[i] * Ψ[i]*P[i]) for i=1:k]...,
],t)
...
full_sys = compose(ODESystem([
    ...
    [UpdateLaw.P[i] ~ Identifier.P[i] for i=1:k]...,
    [dP.P[i] ~ Identifier.P[i] for i=1:k]...,
...
[UpdateLaw.Ψ[i] => 0.0 for i=1:k]...,
[Identifier.ϕ[i] => 0.0 for i=1:k]...,
[dP.P[i] => 1.0 for i=1:k]...,
]
...
if is_plotting
    ...
    perrtrnc = plot(sol.t[err_trunc_st:err_trnc_end],
        abs.(sol[NeurTrue.V,err_trunc_st:err_trnc_end].-sol[Identifier.Vh,err_trunc_st:err_trnc_end]),label=L"|v-\hat{v}\,|",legend=:bottomleft,ylabel="[mV]",size=1.2 .* (600,450))
